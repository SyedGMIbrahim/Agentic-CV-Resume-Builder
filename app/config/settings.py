...
class Settings(BaseSettings):
    ...
    openai_temperature: float = Field(
        default=0.7,
        description="Temperature setting for OpenAI model creativity"
    )
    ...
    def is_user_openai_available(self, user_api_key: str) -> bool:
        """Check if user-provided OpenAI API key is available."""
        return user_api_key is not None and len(user_api_key.strip()) > 0
