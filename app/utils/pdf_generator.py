...
class PDFGenerator:
    ...
    def __init__(self, session_id: str = None):
        ...
        if session_id:
            ...
            # When using a session_id based directory, don't remove it automatically on object deletion
            self._owned_temp_dir = False
        else:
            self.temp_dir = tempfile.mkdtemp(prefix="cv_builder_")
            # mkdtemp created this directory and should be removed on cleanup
            self._owned_temp_dir = True
        print(f"PDFGenerator using temp directory: {self.temp_dir}")
        # Resolve LaTeX binary and cache installation state
        self.pdflatex_bin = self._get_pdflatex_cmd()
        # Store last error text from LaTeX probing/compilation for debugging/UI
        self.last_latex_error: Optional[str] = None
        self.latex_installed = self.ensure_latex_installed()

    def _get_pdflatex_cmd(self) -> Optional[str]:
        """Resolve pdflatex binary path, trying PATH and common macOS install location."""
        # 1) Allow explicit override via environment variable(s)
        env_keys = ['PDFLATEX_CMD', 'PDFLATEX_PATH', 'PDFTEX_PATH']
        for k in env_keys:
            val = os.environ.get(k)
            if val:
                # If it's a directory, append pdflatex
                if os.path.isdir(val):
                    candidate = os.path.join(val, 'pdflatex')
                else:
                    candidate = val
        ...
        # 3) Common macOS/Linux TeX paths (use glob for wildcards)
        mac_tex_paths = [
            '/Library/TeX/texbin/pdflatex',
            '/usr/texbin/pdflatex',
            '/usr/local/texlive/202*/bin/*/pdflatex',
            '/usr/local/texlive/*/bin/*/pdflatex',
            '/usr/bin/pdflatex',
        ]
        import glob
        for p in mac_tex_paths:
            if '*' in p:
                matches = glob.glob(p)
                if matches:
                    for m in matches:
    ...
    def ensure_latex_installed(self) -> bool:
        ...
        try:
            cmd = self.pdflatex_bin or self._get_pdflatex_cmd()
            if not cmd:
                self.last_latex_error = "pdflatex binary not found in PATH or common locations."
                return False
            ...
            try:
                ...
                self.last_latex_error = (out + '\n' + err).strip()
            except Exception:
                self.last_latex_error = None

            if ok:
                # Cache resolved binary for subsequent calls
                self.pdflatex_bin = cmd
            return ok
        except (subprocess.TimeoutExpired, FileNotFoundError):
            self.last_latex_error = "Timeout or FileNotFound when trying to probe pdflatex"
            return False
    ...
    def generate_pdf_from_latex(self, latex_code: str) -> Optional[str]:
        ...
        if not self.ensure_latex_installed():
            msg = "LaTeX (pdflatex) not available. Please install a TeX distribution (e.g. `texlive-full` on Linux or MacTeX on macOS) or set the PDFLATEX_CMD environment variable to the pdflatex binary."
            try:
                st.error(msg)
                if self.last_latex_error:
                    with st.expander("LaTeX probe output"):
                        st.code(self.last_latex_error)
            except Exception:
                print(msg)
                if self.last_latex_error:
                    print("LaTeX probe output:\n", self.last_latex_error[:2000])
            return None
    ...
    def compile_latex_to_pdf(self, latex_code: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                raise PermissionError(f"Temp directory not writable: {self.temp_dir} ({e})")

            # Ensure LaTeX binary is available
            cmd = self.pdflatex_bin or self._get_pdflatex_cmd()
            if not cmd:
                self.last_latex_error = "pdflatex binary not found when attempting compile"
                try:
                    st.error("LaTeX not installed. Please install texlive-full or configure pdflatex path.")
                except Exception:
                    print("LaTeX not installed. Please install texlive-full or configure pdflatex path.")
                return None
            ...
            if os.path.exists(pdf_path):
                ...
                # Save last error for UI or later inspection
                self.last_latex_error = error_log
            ...
            self.last_latex_error = "LaTeX compilation timed out"
            try:
    ...
    def cleanup(self):
        ...
        try:
            # Only remove temp_dir if this instance created it
            if getattr(self, '_owned_temp_dir', False) and os.path.exists(self.temp_dir):
