...
def render_pdf_preview(force_update: bool = False):
    ...
    # Diagnostics panel to help troubleshooting when preview/download don't appear
    try:
        with st.expander("PDF generation diagnostics", expanded=False):
            st.write("pdf_path:", st.session_state.get('pdf_path'))
            pdf_path = st.session_state.get('pdf_path')
            try:
                st.write("pdf exists:", bool(pdf_path and os.path.exists(pdf_path)))
            except Exception as e:
                st.write("pdf exists: could not check (error)", e)

            pdf_gen = st.session_state.get('pdf_generator')
            if pdf_gen:
                try:
                    st.write("pdflatex_cmd:", getattr(pdf_gen, 'pdflatex_bin', None))
                    st.write("latex_installed:", getattr(pdf_gen, 'latex_installed', None))
                    st.write("last_latex_error (truncated):", (getattr(pdf_gen, 'last_latex_error', None) or '')[:1000])
                except Exception as e:
                    st.write("Could not read pdf_generator diagnostics:", e)
            else:
                st.write("pdf_generator: not set in session_state")
    except Exception:
        # Diagnostics panel must never break the preview flow
        pass
    ...
    with st.spinner("Generating PDF from your data..."):
        try:
            ...
            if pdf_path:
                    ...
                    # keep a reference to the generator to prevent cleanup of temp files
