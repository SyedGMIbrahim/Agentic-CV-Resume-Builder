...
def render_latex_editor_section() -> bool:
    ...
    # Import and render PDF preview
    try:
        from components.pdf_preview import render_pdf_preview
        render_pdf_preview()
    except ImportError:
        st.info("PDF preview will be shown here after compilation.")
...
def reframe_project_description(project: Dict[str, Any], index: int):
    ...
    if openai_client.is_available():
        with st.spinner("🤖 AI is reframing your project description..."):
            reframed = openai_client.reframe_content(
...
def regenerate_professional_summary(job_description: str, user_feedback: str):
    ...
    if openai_client.is_available():
        with st.spinner("🤖 AI is improving your professional summary..."):
            ...
            improved_summary = openai_client.generate_professional_summary_with_feedback(
                user_data,
                job_description,
                st.session_state.ai_generated_summary,
                user_feedback
...
def analyze_job_posting(job_description: str):
    ...
    if openai_client.is_available():
        with st.spinner("📊 Analyzing job posting..."):
            analysis = openai_client.analyze_job_posting(job_description)
    ...
    try:
        ...
        if latex_code:
            ...
            if pdf_path:
                ...
                # keep a reference to the generator so temp files aren't removed
...
from components.latex_editor import compile_latex_to_pdf, save_latex_code
