...
def get_user_api_key():
    """Get OpenAI API key from environment only - no user input required"""
    # Always use the API key from .env file
    if settings.openai_api_key:
        return settings.openai_api_key
    
    # If no API key in .env, return empty string (AI features will be disabled)
    return ""
...
def render_resume_settings():
    ...
    with st.expander("PDF Generation Settings", expanded=False):
        ...
        # Don't manually set session state - the widget already handles it