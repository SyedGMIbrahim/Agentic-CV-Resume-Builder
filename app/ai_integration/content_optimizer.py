...
class ContentOptimizer:
    ...
    def optimize_resume_for_job(
        ...
        if not self.openai_client.is_available():
            st.write(f"Debug: OpenAI client not available. User API key provided: {self.api_key is not None}")
            st.write(f"Debug: API key length: {len(self.api_key) if self.api_key else 0}")
            st.write(f"Debug: OpenAI client object: {self.openai_client.client}")
        ...
        with st.spinner("🤖 Generating professional summary..."):
            summary = self.openai_client.generate_professional_summary(
        ...
        with st.spinner("🤖 Analyzing section relevance..."):
            section_relevance = self.openai_client.analyze_section_relevance(user_data, job_description)
    ...
    def get_project_recommendations(
        ...
        selected_projects = self.openai_client.select_best_projects(
            projects, job_description, max_projects=3
        )
    ...
    def get_skills_gap_analysis(
        ...
        recommended_skills = self.openai_client.generate_skills_recommendations(
    ...
    def optimize_single_project(
        ...
        if project.get('description'):
            optimized_description = self.openai_client.optimize_content_for_job(
                project['description'],
                "project description",
                job_description
            )
    ...
    def get_optimization_suggestions(
        ...
        if self.openai_client.is_available() and job_description:
    ...
    def select_best_experiences_with_scores(
        ...
        """
        if not self.openai_client.is_available() or not experiences:
