...
class OpenAIClient:
    """Client for interacting with OpenAI API using Pydantic settings (keeps class name for compatibility)"""
    ...
    def generate_professional_summary(
        ...
        try:
            ...
            # Call OpenAI API with retries for word count compliance
            max_attempts = 3
            for attempt in range(max_attempts):
                response = self.client.chat.completions.create(
                    model=self.config.get("model", "gpt-4o-mini"),
                    max_completion_tokens=self.config.get("max_tokens", 2000),
                    messages=[
                        {
                            "role": "system",
